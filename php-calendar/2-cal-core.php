...
class Calendar {
  ...
  function save ($start, $end, $txt, $color, $id=null) {
    ...
    if ($id==null) {
      $sql = "INSERT INTO `agenda` (`evt_start`, `evt_end`, `evt_text`, `evt_color`) VALUES (?,?,?,?)";
      $data = [$start, $end, $txt, $color];
    } else {
      $sql = "UPDATE `agenda` SET `evt_start`=?, `evt_end`=?, `evt_text`=?, `evt_color`=? WHERE `evt_id`=?";
  ...
  // (E) DELETE EVENT
  function del ($id) {
    return $this->exec("DELETE FROM `agenda` WHERE `evt_id`=?", [$id]);
  }
